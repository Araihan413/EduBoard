        ...
        {isMidGame ? (
          ...
          >
            ...
            <div className="flex justify-center mb-2 sm:mb-5">
              <motion.div 
                animate={isMobile ? { scale: [1, 1.05, 1] } : { rotate: [0, -10, 10, 0], scale: [1, 1.12, 1] }} 
                transition={isMobile ? { duration: 2.5, repeat: Infinity, ease: "easeInOut" } : { duration: 2, repeat: Infinity }}
              >
                <Trophy className={`w-10 h-10 sm:w-15 sm:h-15 text-yellow-400 ${isMobile ? "" : "filter drop-shadow-[0_0_16px_rgba(250,204,21,0.6)]"}`} />
              </motion.div>
            </div>
            ...
            <div className={`inline-flex items-center gap-1.5 sm:gap-3 bg-white/10 px-3 py-2 sm:px-5 sm:py-1.5 rounded-full border border-white/20 ${isMobile ? "" : "backdrop-blur-md"}`}>
