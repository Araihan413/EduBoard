...
function CardFrontFace({
  ...
  return (
    <div className="absolute inset-0 bg-white rounded-[2.5rem] landscape-mobile:rounded-[1.25rem] flex flex-col p-5 md:p-8 landscape-mobile:p-3 overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.2)] border-2 border-slate-100">
      {/* Corner Accent */}
      ...
      <div className="relative z-10 flex items-center justify-between mb-1 pb-1 border-b border-slate-100">
        <div>
          <p className={`text-[10px] landscape-mobile:text-[8px] font-black uppercase tracking-[0.4em] mb-1 ${accent.text}`}>
            {isUnderReview ? "PENILAIAN GURU" : `KARTU ${cardType}`}
          </p>
          {!isUnderReview && displayCard && (
            <div className="flex items-center gap-2">
              <span className="text-3xl landscape-mobile:text-xl font-black text-slate-900 tracking-tighter">{displayCard.points}</span>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Poin</span>
            </div>
          )}
        </div>
        {isTimerRunning && !isUnderReview && (
          <div className={`w-14 h-14 landscape-mobile:w-9 landscape-mobile:h-9 rounded-2xl landscape-mobile:rounded-lg flex flex-col items-center justify-center border-2 border-white shadow-lg ${timer <= 5 ? "bg-red-500 text-white animate-pulse" : "bg-slate-900 text-white"}`}>
            <span className="text-xl landscape-mobile:text-xs font-black font-mono leading-none">{timer}</span>
            <span className="text-[7px] landscape-mobile:text-[5px] font-black uppercase tracking-widest opacity-60">Sec</span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 flex flex-col relative z-10 min-h-0">
        <div className="flex-1 overflow-y-auto overscroll-contain min-h-0 pr-1 pb-6 landscape-mobile:pb-2">
          {isUnderReview ? (
            <div className="space-y-3 md:space-y-4 landscape-mobile:space-y-2">
              ...
              {pendingReviews
                ...
                .slice(0, 1)
                .map((rev) => (
                  <div key={rev.id} className="space-y-3 md:space-y-4 landscape-mobile:space-y-2">
                    <div className="text-center mb-1 md:mb-2 landscape-mobile:mb-0">
                      <p className="text-[8px] md:text-[10px] landscape-mobile:text-[7px] font-black text-slate-400 uppercase tracking-[0.4em] mb-0.5 md:mb-1">MENILAI JAWABAN</p>
                      <h3 className="text-sm md:text-base landscape-mobile:text-xs font-black text-slate-900 tracking-tight">Tim {rev.groupName}</h3>
                    </div>

                    {/* Pertanyaan */}
                    <div className="p-3 landscape-mobile:p-2 bg-slate-50 border border-slate-100 rounded-xl landscape-mobile:rounded-lg text-left">
                      <p className="text-[7px] md:text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Pertanyaan:</p>
                      <p className="text-xs md:text-sm landscape-mobile:text-[10px] font-bold text-slate-800 leading-tight">{rev.question || displayCard?.text}</p>
                    </div>

                    {/* Isi Jawaban Siswa */}
                    <div className="p-3 landscape-mobile:p-2 bg-orange-50/50 border border-orange-100 rounded-xl landscape-mobile:rounded-lg text-left relative overflow-hidden">
                      <p className="text-[7px] md:text-[8px] font-black text-orange-500 uppercase tracking-widest mb-0.5">Jawaban Siswa:</p>
                      <p className="text-sm md:text-base landscape-mobile:text-xs font-bold text-slate-800 italic leading-relaxed relative z-10">&ldquo;{rev.answer}&rdquo;</p>
                    ...
                    {role === "guru" && displayCard?.answerKey && (
                      <div className="p-3 landscape-mobile:p-2 bg-blue-50/50 rounded-xl landscape-mobile:rounded-lg border border-blue-100 text-left">
                        <p className="text-[7px] md:text-[8px] font-black text-blue-400 uppercase tracking-widest mb-0.5">Referensi Kunci Jawaban:</p>
                        <p className="text-xs md:text-sm landscape-mobile:text-[10px] font-bold text-blue-900 leading-tight">{displayCard.answerKey}</p>
                      </div>
                    )}
                  </div>
                ))}
            ...
            <div>
              <p className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.3em] mb-2 landscape-mobile:mb-1 landscape-mobile:text-[8px]">ISI KARTU</p>
              <p className="text-base md:text-lg landscape-mobile:text-xs font-bold text-zinc-900 leading-snug mb-4 md:mb-6 landscape-mobile:mb-2">&ldquo;{displayCard?.text}&rdquo;</p>
              ...
              {isMyTurn && activeGroup && (
                <div className="mt-3 pt-3 border-t border-zinc-100 landscape-mobile:mt-1.5 landscape-mobile:pt-1.5">
                  {displayCard?.type === "DASAR" && displayCard.options ? (
                    ...
                    <div className="space-y-2 md:space-y-3 landscape-mobile:space-y-1.5">
                      {isUnderReview ? (
                        <div className={`flex flex-col items-center justify-center py-6 md:py-10 landscape-mobile:py-3 ${accent.light} rounded-2xl border-2 border-dashed ${cardType === "TANTANGAN" ? "border-red-200" : "border-orange-200"} animate-pulse`}>
                          <div className={`w-10 h-10 landscape-mobile:w-7 landscape-mobile:h-7 ${cardType === "TANTANGAN" ? "bg-red-100" : "bg-orange-100"} rounded-full flex items-center justify-center mb-2 md:mb-4 landscape-mobile:mb-1`}>
                            <Rocket className={`w-5 h-5 landscape-mobile:w-3.5 landscape-mobile:h-3.5 ${accent.text} animate-bounce`} />
                          </div>
                          <p className={`${cardType === "TANTANGAN" ? "text-red-900" : "text-orange-900"} font-black text-center uppercase tracking-widest text-[10px] md:text-xs landscape-mobile:text-[9px]`}>Menunggu Penilaian Guru</p>
                        ...
                        <>
                          {displayCard?.type === "PEMAHAMAN" ? (
                            <textarea
                              className="w-full min-h-[80px] md:min-h-[100px] landscape-mobile:min-h-[44px] bg-white border-2 border-zinc-900 rounded-xl landscape-mobile:rounded-lg px-4 md:px-5 landscape-mobile:px-3 py-3 md:py-4 landscape-mobile:py-1.5 text-zinc-900 text-sm md:text-base landscape-mobile:text-xs font-bold focus:outline-none focus:ring-4 focus:ring-orange-500/15 resize-none shadow-inner"
                              placeholder="Ketik jawabanmu..."
                              value={tantanganText}
                              onChange={(e) => setTantanganText(e.target.value)}
                            />
                          ) : (
                            <div className="py-4 md:py-8 px-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-center mb-1 md:mb-2 landscape-mobile:py-2">
                              <p className="text-[11px] md:text-xs landscape-mobile:text-[10px] font-bold text-slate-500">Jawab pertanyaan di atas secara lisan, lalu klik tombol di bawah jika sudah selesai!</p>
                            </div>
                          )}
                          <button
                            onClick={() => {
                            ...
                            className="w-full py-2.5 md:py-3.5 landscape-mobile:py-1.5 mb-2 md:mb-4 landscape-mobile:mb-1 rounded-xl landscape-mobile:rounded-lg bg-zinc-900 text-white font-black text-xs md:text-sm landscape-mobile:text-[10px] tracking-[0.15em] md:tracking-[0.2em] landscape-mobile:tracking-wider uppercase hover:bg-zinc-800 disabled:opacity-20 transition-all shadow-[4px_4px_0_0_rgba(0,0,0,0.2)] disabled:cursor-not-allowed"
                          >
                    ...
                    <button
                      ...
                      className="w-full py-2.5 md:py-4 landscape-mobile:py-1.5 mb-2 md:mb-4 landscape-mobile:mb-1 rounded-xl landscape-mobile:rounded-lg bg-zinc-900 text-white text-xs md:text-base landscape-mobile:text-[10px] font-black tracking-widest uppercase hover:bg-zinc-800 transition-all shadow-[4px_4px_0_0_rgba(0,0,0,0.2)] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      LANJUT
                    </button>
                  )}
              ...
              {(role === "guru" || !isMyTurn) && (
                <div className="mt-3 landscape-mobile:mt-1.5 bg-zinc-100 border border-zinc-200 p-4 landscape-mobile:p-2 rounded-xl landscape-mobile:rounded-lg text-center">
                  <p className="text-[8px] font-black text-zinc-400 uppercase tracking-widest mb-0.5">STATUS</p>
                  <p className="text-sm landscape-mobile:text-xs font-black text-zinc-800">MENUNGGU TIM {activeGroup?.name ?? "..."}</p>
                </div>
              )}
            </div>
          )}
        ...
        {/* Grading Section */}
        {isUnderReview && (
          <div className="mt-3 md:mt-6 landscape-mobile:mt-1.5 pt-3 md:pt-6 landscape-mobile:pt-1.5 border-t border-zinc-100">
            {role === "guru" ? (
              <div className="space-y-2 md:space-y-3 landscape-mobile:space-y-1">
                <div className="flex items-center gap-1.5 text-orange-600 mb-1">
                  <AlertCircle className="w-3.5 h-3.5 landscape-mobile:w-3 landscape-mobile:h-3 animate-pulse" />
                  <span className="text-[9px] landscape-mobile:text-[7.5px] font-black uppercase tracking-widest">Berikan Penilaian Sekarang</span>
                </div>

                <div className="grid grid-cols-3 gap-2 landscape-mobile:gap-1.5">
                  {[
                    { label: "Salah",   score: 0,                                          icon: <XCircle     className="w-5 h-5 landscape-mobile:w-4 landscape-mobile:h-4 text-red-500 group-hover:scale-110 transition-transform" />,    cls: "border-red-200 bg-red-50 hover:bg-red-100",         text: "text-red-700"     },
                    { label: `Sebagian (+${Math.floor((activeReview?.points ?? 10) / 2)})`, score: Math.floor((activeReview?.points ?? 10) / 2), icon: <div className="w-5 h-5 landscape-mobile:w-4 landscape-mobile:h-4 rounded-full border-2 border-orange-400 border-t-transparent animate-spin-slow group-hover:animate-none" />, cls: "border-orange-200 bg-orange-50 hover:bg-orange-100", text: "text-orange-700" },
                    { label: `Tepat (+${activeReview?.points ?? 10})`,                     score: activeReview?.points ?? 10,                    icon: <CheckCircle2 className="w-5 h-5 landscape-mobile:w-4 landscape-mobile:h-4 text-emerald-500 group-hover:scale-110 transition-transform" />, cls: "border-emerald-200 bg-emerald-50 hover:bg-emerald-100", text: "text-emerald-700" },
                  ].map(({ label, score, icon, cls, text }) => (
                    <button
                      key={label}
                      disabled={isSubmitting || isGrading}
                      onClick={() => activeReview && handleGrade(activeReview.id, score)}
                      className={`flex flex-col items-center gap-0.5 py-2 px-1 landscape-mobile:py-1 rounded-xl landscape-mobile:rounded-lg border-2 transition-all shadow-sm group disabled:opacity-50 disabled:grayscale ${cls}`}
                    >
                      {icon}
                      <span className={`font-black text-[8px] md:text-[9px] landscape-mobile:text-[7px] tracking-tighter md:tracking-tight uppercase ${text}`}>{label}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <motion.div
                ...
                className="bg-indigo-50 border border-indigo-100 p-4 md:p-6 landscape-mobile:p-2.5 rounded-2xl landscape-mobile:rounded-xl text-center shadow-md shadow-indigo-500/5"
              >
                <div className="flex flex-col items-center gap-2 md:gap-3 landscape-mobile:gap-1">
                  <div className="relative">
                    <div className="absolute inset-0 bg-indigo-400 rounded-full blur-lg opacity-20 animate-pulse" />
                    <div className="w-10 h-10 landscape-mobile:w-7 landscape-mobile:h-7 bg-white rounded-full flex items-center justify-center shadow border border-indigo-50 relative z-10">
                      <Disc3 className="w-5 h-5 landscape-mobile:w-4 landscape-mobile:h-4 text-indigo-500 animate-spin-slow" />
                    </div>
                  </div>
                  <div>
                    <p className="text-[8px] font-black text-indigo-400 uppercase tracking-[0.25em] mb-1">PROSES PENILAIAN</p>
                    <h4 className="text-base font-black text-indigo-900 tracking-tight landscape-mobile:text-xs">Menunggu Guru Menilai...</h4>
                    <p className="text-[10px] landscape-mobile:text-[8px] font-medium text-indigo-600/70 mt-1 leading-relaxed">
